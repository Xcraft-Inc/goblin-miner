...
class CodeMiner extends Elf {
  ...
  async loadModule(module, instructFileName) {
    ...
    excludes = excludes.filter((entry) => !entry.startsWith('!'));
    for (const file of domainFiles) {
      if (includes.includes(file.name)) {
  ...
  async generate(input, output = 'README.md') {
    if (this._type === 'translate') {
      return this._generateTranslation(input, output);
    }
    return this._generateDocumentation(input, output);
  }
  ...
  async _generateTranslation(inputFile, outputFile) {
    this.log.dbg('Generating translation...');

    const instructFileName = path.basename(inputFile);
    const instructName = instructFileName.split('.').slice(0, -1).join('.');
    const module = path.dirname(inputFile);
    const libPath = path.isAbsolute(module)
      ? module
      : path.join(this._projectPath, 'lib', module);
    ...
    const absolute = path.isAbsolute(inputFile)
      ? inputFile
    ...
    const output = path.isAbsolute(outputFile)
      ? outputFile

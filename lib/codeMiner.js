...
function loadInstruction(libPath, type, fileName) {
  let instructsPath;
  /* Target module */
  ...
  /* Miner */
  instructsPath = path.join(__dirname, 'instructions', type, fileName);
  return fse.readFileSync(instructsPath, 'utf8');
}
...
class CodeMiner extends Elf {
  ...
  async generate(module, instructFile = 'README.md') {
    ...
    const sysPrompt = loadPrompt(`${this._type}.md`);
    const sysInstruct = loadInstruction(libPath, this._type, instructFileName);

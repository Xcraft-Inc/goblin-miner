...
class CodeMiner extends Elf {
  ...
  async loadModule(goblinName, excludes) {
    ...
    for (const file of domainFiles) {
      ...
      if (
        excludes
          .filter((exclude) => exclude.endsWith('/'))
          .includes(path.basename(file.parentPath))
      ) {
        continue;
      }
  ...
  async generateDocumentation(
    ...
    for (const module of modules) {
      ...
      const fileName = 'instruct-' + instructFile;
      ...
      if (fse.existsSync(mdFilePath)) {
        ...
        prompt += `## Documentation précédent\n\n`;
        prompt += '```md\n';
        prompt += content;
        prompt += '```\n\n';
      }
  ...
  async generateReadme(modules, excludes = []) {

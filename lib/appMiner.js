...
class AppMiner extends Elf.Alone {
  ...
  async main() {
    ...
    options.input = minerConfig.modules.doc;
    options.output = minerConfig.instructs.doc;

    // Not in package.json (needed only when using AppMiner)
    const {projectPath, appArgs} = require('xcraft-core-host');

    /*
     * -t, --type         (default: xcraft) (choices: xcraft, cxx, dotnet, translate)
    ...
    for (const input of options.input) {
      for (const output of options.output) {
        await codeMiner.generate(input, output);
      }
    }
  }
}
